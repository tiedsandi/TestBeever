...
function counter(arr) {
    ...
    let result = {}; // buat object kosong
    for (let i = 0; i < arr.length; i++) { // looping untuk mencari angka yang muncul
        // console.log(arr[i]);
        if (result[arr[i]]) { // jika angka yang dicari sudah ada di object
            result[arr[i]]++; // maka tambahkan 1
        } else { // jika angka yang dicari belum ada di object
            result[arr[i]] = 1; // maka buat object baru dengan keynya angka yang dicari dan valuenya 1
        }
    }
    return result;
}
